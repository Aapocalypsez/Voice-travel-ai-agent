        ...
        while (true) {
          ...
          try {
            ...
            if ("error" in chunkJson) {
              ...
              const errorMessage2 = `got status: ${status2}. ${JSON.stringify(chunkJson)}`;
...
var APIError = class _APIError extends GeminiNextGenAPIClientError {
  ...
  static makeMessage(status2, error, message) {
    ...
    const statusText = status2 ? `${status2} ` : "";
  ...
  static generate(status2, errorResponse, message, headers) {
    ...
    if (status2 === 400) {
      return new BadRequestError(status2, errorResponse, message, headers);
    }
    if (status2 === 401) {
      return new AuthenticationError(status2, errorResponse, message, headers);
    }
    if (status2 === 403) {
      return new PermissionDeniedError(status2, errorResponse, message, headers);
    }
    if (status2 === 404) {
      return new NotFoundError(status2, errorResponse, message, headers);
    }
    if (status2 === 409) {
      return new ConflictError(status2, errorResponse, message, headers);
    }
    if (status2 === 422) {
      return new UnprocessableEntityError(status2, errorResponse, message, headers);
    }
    if (status2 === 429) {
      return new RateLimitError(status2, errorResponse, message, headers);
    }
    if (status2 >= 500) {
      return new InternalServerError(status2, errorResponse, message, headers);
    }
    return new _APIError(status2, errorResponse, message, headers);
  }
};
...
// public/app.js
var LANGUAGES = [
  { code: "English", englishName: "English", label: "English", flag: "\u{1F1FA}\u{1F1F8}", currency: "USD", locale: "en-US", rate: 1 },
  { code: "Espa\xF1ol", englishName: "Spanish", label: "Espa\xF1ol", flag: "\u{1F1EA}\u{1F1F8}", currency: "EUR", locale: "es-ES", rate: 0.92 },
  { code: "Fran\xE7ais", englishName: "French", label: "Fran\xE7ais", flag: "\u{1F1EB}\u{1F1F7}", currency: "EUR", locale: "fr-FR", rate: 0.92 },
  { code: "Deutsch", englishName: "German", label: "Deutsch", flag: "\u{1F1E9}\u{1F1EA}", currency: "EUR", locale: "de-DE", rate: 0.92 },
  { code: "\u0939\u093F\u0928\u094D\u0926\u0940", englishName: "Hindi", label: "\u0939\u093F\u0928\u094D\u0926\u0940", flag: "\u{1F1EE}\u{1F1F3}", currency: "INR", locale: "hi-IN", rate: 83 },
  { code: "\u65E5\u672C\u8A9E", englishName: "Japanese", label: "\u65E5\u672C\u8A9E", flag: "\u{1F1EF}\u{1F1F5}", currency: "JPY", locale: "ja-JP", rate: 157 },
  { code: "Portugu\xEAs", englishName: "Portuguese", label: "Portugu\xEAs", flag: "\u{1F1E7}\u{1F1F7}", currency: "BRL", locale: "pt-BR", rate: 5.1 }
];
var DEFAULT_LANG = LANGUAGES[0];
...
var STRINGS = {
  ...
  Espa\u00F1ol: { flights: "Vuelos", hotels: "Hoteles cercanos", perNight: "/ noche", nonstop: "Directo", stop: "escala", stops: "escalas", viewDetails: "Ver detalles", departs: "Sale", arrives: "Llega", duration: "Duraci\xF3n", rating: "Valoraci\xF3n" },
  Fran\u00E7ais: { flights: "Vols", hotels: "H\xF4tels \xE0 proximit\xE9", perNight: "/ nuit", nonstop: "Direct", stop: "escale", stops: "escales", viewDetails: "Voir les d\xE9tails", departs: "D\xE9part", arrives: "Arriv\xE9e", duration: "Dur\xE9e", rating: "Note" },
  Deutsch: { flights: "Fl\xFCge", hotels: "Hotels in der N\xE4he", perNight: "/ Nacht", nonstop: "Nonstop", stop: "Stopp", stops: "Stopps", viewDetails: "Details ansehen", departs: "Abflug", arrives: "Ankunft", duration: "Dauer", rating: "Bewertung" },
  ...
  Portugu\u00EAs: { flights: "Voos", hotels: "Hot\xE9is pr\xF3ximos", perNight: "/ noite", nonstop: "Direto", stop: "parada", stops: "paradas", viewDetails: "Ver detalhes", departs: "Partida", arrives: "Chegada", duration: "Dura\xE7\xE3o", rating: "Avalia\xE7\xE3o" }
};
function t(lang, key) {
...
var PERKS = ["Free Wi-Fi", "Breakfast included", "Pool", "Gym", "Airport shuttle", "Spa"];
function searchHotels(destination) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const perks = [.../* @__PURE__ */ new Set([PERKS[(seed + i) % PERKS.length], PERKS[(seed + i + 2) % PERKS.length]])];
    options.push({
      name: n.name,
      area: n.area,
...
var AudioRecorder = class {
  ...
  async start() {
    ...
    this.ctx = new AudioContext({ sampleRate: 16e3 });
...
var AudioPlayer = class {
  ...
  ensureCtx() {
    if (!this.ctx || this.ctx.state === "closed") {
      this.ctx = new AudioContext({ sampleRate: 24e3 });
      this.nextStartTime = 0;
    }
    return this.ctx;
  }
  enqueue(base64) {
    ...
    const buffer = ctx.createBuffer(1, float32.length, 24e3);
...
var langFlag = $("langFlag");
var btnMic = $("btnMic");
var iconMic = $("iconMic");
var iconMicOff = $("iconMicOff");
var iconSpinner = $("iconSpinner");
var orbRing1 = $("orbRing1");
var orbRing2 = $("orbRing2");
var statusLine = $("statusLine");
var btnEnd = $("btnEnd");
var suggestions = $("suggestions");
var resultsSection = $("resultsSection");
var flightsCard = $("flightsCard");
var hotelsCard = $("hotelsCard");
var transcriptSection = $("transcriptSection");
var transcriptList = $("transcriptList");
var activityList = $("activityList");
var dockedMic = $("dockedMic");
var btnMicDocked = $("btnMicDocked");
var iconMicDocked = $("iconMicDocked");
var iconMicOffDocked = $("iconMicOffDocked");
var iconSpinnerDocked = $("iconSpinnerDocked");
var dockRing1 = $("dockRing1");
var dockRing2 = $("dockRing2");
var statusLineDocked = $("statusLineDocked");
var btnEndDocked = $("btnEndDocked");
var status = "idle";
var muted = false;
...
var uiPhase = "idle";
var hasTransitionedToSplit = false;
var savedLang = localStorage.getItem("avery-lang");
if (savedLang) {
...
function renderActivity() {
  ...
  for (let i = logs.length - 1; i >= 0; i--) {
    ...
    html += `<div class="log-entry"><span class="log-ts">${ts}</span><span class="${cls}">${escapeHtml(l.detail)}${l.latencyMs != null ? ` <span class="log-latency">\xB7 ${l.latencyMs}ms</span>` : ""}</span></div>`;
  }
...
function renderFlights(result) {
  ...
  <p class="result-card-subheading">
    ...
    <span>\xB7 ${escapeHtml(result.date)}</span>
  </p>
  ...
  for (const opt of result.options) {
    ...
    html += `<button class="item-card" onclick="this.querySelector('.item-details')?.classList.toggle('hidden')">
      <div class="item-row">
        <div><p class="item-name">${escapeHtml(opt.airline)}</p><p class="item-meta">${opt.flightNumber} \xB7 ${stopsLabel}</p></div>
  ...
  if (message.toolCall?.functionCalls?.length) {
    const functionResponses = message.toolCall.functionCalls.map((fc) => {
      if (fc.name === "search_flights") {
        ...
        renderUI();
        if (!hasTransitionedToSplit) {
          hasTransitionedToSplit = true;
          setPhase("split");
        }
...
async function connect() {
  ...
  try {
    ...
    const ai = new GoogleGenAI2({ apiKey: token, httpOptions: { apiVersion: "v1alpha" } });
